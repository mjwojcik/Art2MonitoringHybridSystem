...
log4m.getLogger.info('vbgm', 'Variational Bayesian Gaussian mixture: running ...'); 
...
[~,~,label] = unique(label);
if converged
    log4m.getLogger.info('vbgm', sprintf('Converged in %d steps.',t-1)); 
else
    log4m.getLogger.info('vbgm', sprintf('Not converged in %d steps.',maxiter)); 
end
...
import GaussianMixtureLib.logsumexp
...
import GaussianMixtureLib.logmvgamma
alpha0 = prior.alpha;
kappa0 = prior.kappa;
m0 = prior.m;
v0 = prior.v;
...
for i = 1:k
    ...
    Xs = bsxfun(@times,bsxfun(@minus,X,xbar(:,i)),sqrtR(:,i)');
    V = chol(Xs*Xs'/nk(i) + diag(ones(1,d)*1e-3)); % add some noise to avoid 
    Q = V/U;
